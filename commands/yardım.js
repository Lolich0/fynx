...
module.exports.run = async (client, message, args) => {
    ...
    .addField(prefix + 'oynat <Şarkı İsmi>', `Belirtilen isimli şarkıları arar ve bulunduğunuz odaya bağlanıp müziği oynatır.`)
    .addField(prefix + 'durdur', `Harmony müzik oynatmayı durdurur ve ses kanalından ayrılır.`)    
    .addField(prefix + 'atla', `Oynatılan olan müziği atlar. Kuyrukta müzik var ise müzikler sırayla oynatılır.`)
    .addField(prefix + 'duraklat', `Oynatılan olan müziği duraklatır.`)
    .addField(prefix + 'devam', `Duraklatılan müziği devam ettirir.`)
    .addField(prefix + 'karıştır', `Müzik kuyruğundaki müzikleri karıştırır.`)
    .addField(prefix + 'döngü', `Müzik kuyruğundaki müzikleri döngü içerisine alır.`)    
